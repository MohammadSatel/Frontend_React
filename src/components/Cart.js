...
const Cart = (props) => {
        ...
        toast.promise(
            axios.post(`${SERVER}/checkout`, { cart: props.cart }, { headers: tokenData })
                .then((res) => {
                    ...
                    toast.success('Checkout successful! The receipt was sent to your email');
                })
                .catch((error) => {
    ...
    // Render the payment component if payment is initiated
    if (props.paynow) {
        return (
            <div>
                ...
                <ListGroup>
                    {props.cart.map(item => (
                        ...
                        >
                            ...
                            {item.amount > 1 && (
                                <CardSubtitle style={{ position: 'absolute', bottom: '15px', right: '25px', fontSize: '12px' }}>
